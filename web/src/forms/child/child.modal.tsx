  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button variant="outline">{isUpdate ? 'Edit Child' : 'Add Child'}</Button>
      </DialogTrigger>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>{isUpdate ? 'Edit Child Information' : 'Add New Child'}</DialogTitle>
          <DialogDescription>
            {isUpdate
        ...
        <ChildForm initialData={initialData} onSubmit={handleSubmit} isUpdate={isUpdate} />
      </DialogContent>
    </Dialog>
  )
}
